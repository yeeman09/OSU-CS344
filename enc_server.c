...
void encrypting_text(char* cipher_buf, char* text_buf, char* key_buf, int length){

  // 5. ENCRPYT
  memset(cipher_buf, '\0', sizeof(cipher_buf));

  // 5.1 Convert plaintext to int
  int plain_int[length - 1];
  int idx = 0;

  for (int i = 0; i < length - 1; i++) {
    for (idx = 0; idx < 27; idx++) {
      if (text_buf[i] == good_char[idx]) plain_int[i] = idx;
    }
  }
  ...
  // 5.3 plus then mod
  for (int i = 0; i < length - 1; i++){
    int good_char_idx = (plain_int[i] + key_int[i]) % 27;
    cipher_buf[i] = good_char[good_char_idx];
  }
  cipher_buf[length] = '\0';
  ...
  while(1) {
    ...
    switch(spawnPid){
      ...
      case 0:
        ...
        if (pair_rec < 0) error("SERVER: pair key received failed.");
        if (strcmp(pair_key, "ENC") != 0) {
          char res[2] = "n";
          int pair_result = send(connectionSocket, res, sizeof(res), 0);
          if (pair_result < 0) error("SERVER: pair result send failed");
        }
        ...
        if (charsRead < 0) error("SERVER ERROR reading from socket - text length.");
        //printf("SERVER: text length: %d\n", text_length);

        // 3. Get original plaintext from client
        ...
        //printf("SERVER plaintxt buffer: %s\n", text_buffer);
        ...
        //printf("SERVER key text buffer: %s\n", key_buffer);

        // 5. Encrpyt 
        encrypting_text(enc_text, text_buffer, key_buffer, text_length);

        // 6. Send back to client
        charsWritten = send_all(connectionSocket, enc_text, text_length);
